...
from models.qwen.qwen_generation_utils import (
    HistoryType,
    make_context,
    decode_tokens,
    get_stop_words_ids,
...
from models.base_mp import BaseParallelLayer, RowParallelLinear, ColumnParallelLinear, QKVColumnParallelLinear
...
class QWenAttention(BaseParallelLayer):
    def __init__(self, config):
        ...
        self.c_attn = QKVColumnParallelLinear(config.hidden_size, 
                                              3 * self.projection_size, 
                                              gather_input=False,
                                              gather_output=False)

        self.c_proj = RowParallelLinear(config.hidden_size, 
    ...
    def turn_global_weights_to_local(self):
        ...
        self.split_size = self.split_size // self.world_size
    ...
    def forward(
        ...
        query = self._split_heads(query, self.local_head, self.head_dim)
        key = self._split_heads(key, self.local_head, self.head_dim)
        value = self._split_heads(value, self.local_head, self.head_dim)
        ...
        context_layer = self._merge_heads(
            attn_output, self.local_head, self.head_dim
        )
...
class QWenMLP(BaseParallelLayer):
    def __init__(self, config):
        super().__init__()
        self.w1 = ColumnParallelLinear(config.hidden_size, 
                                              config.intermediate_size // 2, 
                                              bias=not config.no_bias,
                                              gather_input=False,
                                              gather_output=False,)
        self.w2 = ColumnParallelLinear(config.hidden_size, 
                                              config.intermediate_size // 2, 
                                              bias=not config.no_bias,
                                              gather_input=False,
                                              gather_output=False,)
        self.c_proj = RowParallelLinear(config.intermediate_size // 2,
                                           config.hidden_size, 
                                           bias=not config.no_bias,
                                           scatter_input=False,
                                           scatter_output=False,)
